...
unsigned char *keyExpansion(unsigned char *inputkey, unsigned char *_expandedKey) {
  ...
  if (keysize == 256) {
    ...
    while(cur_round < rounds){
      ...
      if(cur_round == 4)
        copyWord(word, &_expandedKey[12]);
      else
        copyWord(word, &_expandedKey[((cur_round-1)*4)]);

      subWord(rotWord(word));
